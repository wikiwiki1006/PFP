...
CONTEXT_CHAR_LIMIT = 4000   # Phase 1 기본값
PHASE2_CONTEXT_LIMIT = 10000  # Phase 2 에이전트(8,9)는 더 많은 컨텍스트 허용


# ── 9개 에이전트 정의 (id, label, color, prompt_fn, max_tokens) ──────────────
def _agents(ev: str, prev_results: list[str], portfolio_str: str,
            context_limit: int = CONTEXT_CHAR_LIMIT) -> list[dict]:
    prev = "\n\n---\n\n".join([r for r in prev_results if r])[-context_limit:]
    return [
        {
...
def _run_contextual_agents(
    ...
    """
    Phase 1 결과(context_texts)를 컨텍스트로 받아 에이전트를 순차 실행.
    에이전트 8(Portfolio Action), 9(Final Verdict) 전용.
    PHASE2_CONTEXT_LIMIT으로 더 많은 컨텍스트를 허용해 진짜 종합 분석을 수행한다.
    """
    all_agents = _agents(ev, context_texts, portfolio_str, context_limit=PHASE2_CONTEXT_LIMIT)
    agent_map = {a["id"]: a for a in all_agents if a["id"] in selected_ids}
...
def render():
    ...
    if run_btn and event_input.strip():
        ...
        # ── 2단계 파이프라인 ──────────────────────────────────────────────
        # Phase 1: 에이전트 1~7 병렬 독립 분석 (도메인 전문가)
        # Phase 2: 에이전트 8~9가 Phase 1 전체 결과를 컨텍스트로 받아 순차 실행
        #          → Agent 8(Portfolio Action), Agent 9(Final Verdict)가
        #            실제 분석 내용을 기반으로 진짜 종합 판단을 내릴 수 있음
        ...
        if phase1_ids:
            p1_labels = ", ".join(
                AGENT_UI_META[i]["label"] for i in phase1_ids if i in AGENT_UI_META
            )
            with st.spinner(f"⚡ Phase 1 — {len(phase1_ids)}개 에이전트 병렬 분석 중... ({p1_labels})"):
                all_results.update(
                    _run_parallel_agents(phase1_ids, event_input, portfolio_str, model)
                )
            ok1 = sum(1 for i in phase1_ids if not all_results.get(i, "").startswith("[오류"))
            st.success(f"✅ Phase 1 완료 ({ok1}/{len(phase1_ids)} 성공) — 도메인 분석 완료")

        if phase2_ids:
            # Phase 1 결과를 에이전트 번호 순으로 합산해 컨텍스트 구성
            p1_texts = [
                all_results[i] for i in sorted(phase1_ids)
                if i in all_results and not all_results[i].startswith("[오류")
            ]
            p2_labels = ", ".join(
                AGENT_UI_META[i]["label"] for i in phase2_ids if i in AGENT_UI_META
            )
            with st.spinner(
                f"🔗 Phase 2 — {len(phase2_ids)}개 에이전트 종합 분석 중... ({p2_labels})\n"
                f"Phase 1 컨텍스트 {len(p1_texts)}개 에이전트 결과 반영"
            ):
                all_results.update(
                    _run_contextual_agents(phase2_ids, event_input, portfolio_str, model, p1_texts)
                )
            ok2 = sum(1 for i in phase2_ids if not all_results.get(i, "").startswith("[오류"))
            st.success(f"✅ Phase 2 완료 ({ok2}/{len(phase2_ids)} 성공) — 종합 시나리오 생성")
        ...
        result_meta = [
            {
                ...
                "label": ag["label"],
                "color": ag["color"],
                "text": all_results.get(ag["id"], "[오류: 결과 없음]"),
            }
        ...
        total_ok = sum(1 for m in result_meta if not m["text"].startswith("[오류"))
        st.success(f"✅ 전체 분석 완료 ({total_ok}/{len(run_agents)}개 에이전트 · 2단계 파이프라인)")

        # ── 카드 그리드 표시 ─────────────────────────────────────────────
        _render_agent_card_grid(run_agents, all_results)
