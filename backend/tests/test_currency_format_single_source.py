...
걸리는 곳은 대개 남의 소유 파일이라 여기서 고칠 수 없다. 그렇다고 허용 목록에
넣으면 **의도된 예외처럼 보인다** — 이건 예외가 아니라 아직 안 고친 결함이다.
...
**항목마다 소유자를 적는다. 소유자 없는 항목은 아무도 고치지 않는다.**

처음 이 원장을 만들었을 때 `trading_signals.py` 가 걸렸는데, 그 파일은 배치표
어디에도 없었다. 그래서 `(소유: 미지정 — 통합 판단 필요)` 라고 적었고, 그 한 줄이
근거가 되어 통합이 소유자를 배정했다. 배정이 되자 그쪽이 고쳤고 항목은 사라졌다.

절차를 여기 적어 두는 이유는, **그 기록이 항목 안에 있으면 항목과 함께
사라지기 때문이다.** 다음에 소유표에 없는 파일이 걸리면 같은 순서로 간다 —
`(소유: 미지정)` 으로 올리고, 통합에 배정을 요청하고, 배정되면 그 이름으로
바꾼다. 소유자를 비워 둔 채 두지 않는다.
"""
