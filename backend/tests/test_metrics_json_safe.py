...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "^VIX column makes vix NaN instead of applying the 18.0 default. "
        "SafeJSONResponse turns it into null, so the screen shows an em dash "
        "rather than failing -- the default is silently lost, not the request. "
        "Delete this marker once the read is wrapped."
    ),
)
def test_all_nan_vix_column_does_not_poison_the_response():
    """`^VIX` 열이 전량 NaN 이면 `vix` 가 NaN 이 되어 기본값 18.0 이 무시된다.

    `vix = float(curr.get("^VIX", 18.0))` 의 기본값은 **열이 없을 때만** 쓰인다.
    열은 있는데 값이 전부 NaN 이면 `.get()` 이 NaN 을 돌려주고, 그 분기에는
    다른 값들과 달리 `_safe()` 가 없다. `ffill()` 도 전량 NaN 열은 못 채운다.

    영향은 요청 실패가 아니다 — `SafeJSONResponse` 가 `null` 로 바꿔 화면에는
    `—` 가 뜬다. **사라지는 건 기본값을 쓰겠다는 의도 쪽이다.** "18.0 을
    적용하려 했는데 실패했다" 와 "VIX 를 조회하지 않았다" 가 화면에서
    구별되지 않는다.

    그리고 안전망이 증상을 지워주기 때문에, 이 누락은 화면만 봐서는 영원히
    발견되지 않는다. yfinance 가 ^VIX 를 빈 열로 주는 것은 실제로 일어난다.
    """
