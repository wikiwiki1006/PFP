...
이 리포에서 가장 자주, 가장 비싸게 반복된 결함이다. 형태는 하나다 —
**뭔가가 실패했는데 호출자에게는 성공처럼 보인다.** 그래서 아무도 모른다.

세 가지로 나타난다.

**(a) 계산 불가를 값으로 위장한다.**
`0%` 는 "보합"이지 "모름"이 아니다. 값이 없으면 `None`/`null` 을 돌려주고 화면에는
`—` 를 그린다. 정렬에서도 값 없는 항목은 뒤로 보낸다. 프론트의 `formatPct()` 가
이미 `null → '—'` 로 처리한다. `?? 0` 을 붙이지 않는다.
> 사고: 섹터 1M/3M/6M 이 전부 `0.0` 으로 나왔는데 데이터가 없어서였다.
> 사용자에게는 "모든 섹터가 보합" 으로 보였다.

**(b) 예외를 삼키고 잘못된 값을 그대로 반환한다.** (a) 보다 나쁘다 — 위장하는 값이
0 이 아니라 **그럴듯한 숫자**라서 의심조차 안 받는다.
> 사고: `total_return_pct` 를 TWRR 로 덮어쓰는 보정이 `except: pass` 안에 있었다.
> 보정이 실패하면 왜곡된 수익률이 정상처럼 화면에 떴다. 로그 한 줄 없었다.

**(c) 안전장치가 실패하면 열린 채로 통과한다.** 보호가 사라졌다는 사실이 어디에도
안 남는다.
> 사고들: 할당량 기록이 `except: pass` 라 `usage_repo` 가 죽으면 전 사용자 무제한.
> 설정 조회가 실패하면 기본값으로 되돌아가 관리자가 켠 제한이 풀림.
> 쓰기 락 획득에 실패하면 락 없이 진행하고 요청은 200 을 반환.
> 자격증명이 하나도 없는데 `init_firebase()` 가 `True` 를 반환.

#### 판단 기준

무언가를 `try` 로 감쌌으면 스스로 물어라 — **이게 실패한 채로 계속 가면, 그걸
누가 언제 알게 되는가?** 답이 "청구서를 보고" 나 "사용자가 신고해서" 면 틀렸다.

- `except: pass` 를 쓰지 마라. 최소한 `logger.warning` 은 남긴다.
- **`print()` 는 로그가 아니다.** 레벨 필터를 안 타고 모듈·시각 맥락도 없어서
  검색도 집계도 안 된다. 운영 로그 파이프라인은 구조화된 출력을 기대하므로
  거기서는 사실상 사라진다. 실패 신호는 `logger` 로 남긴다.
  **모듈에 로거가 없으면 먼저 만들어라** — 로거가 없는 파일에서는 "로그를
  남기자" 라는 수정 자체가 불가능해서, `except: pass` 를 고치러 온 사람이
  그냥 두고 간다. 실제로 `macro.py`·`ticker.py` 가 그 상태였다.
- 안전장치(락·할당량·권한·격리)는 실패 시 **닫는다.** 가용성 때문에 열어야 한다면
  그 판단을 주석에 적고, 열렸다는 사실을 로그에 남긴다.
  `users_repo.is_admin` 이 좋은 예다 — 예외 시 `logger.error` + `return False`.
- 실패한 경로의 반환값이 정상 경로의 반환값과 구별되는지 본다. `{}` 를 돌려주면
  "없음" 과 "조회 실패" 가 같아진다.
- **빈 값을 실패로 취급하지 마라.** `if holdings:` 는 "보유 없음" 인 사용자를
  실패 경로로 보낸다. `if holdings is not None:` 이 맞다.

이건 코드 밖에도 적용된다. 검증 명령을 파이프에 물리면 종료코드가 삼켜져
(§6) 깨진 빌드가 통과로 읽힌다. 같은 결함이다.
...
- **한국은 설·추석이 음력이라 규칙으로 못 구한다.** 그래서 **실제 거래가 있었던
  날**을 시세에서 읽어 캘린더를 만든다(`_krx_trading_days`).
- 소스는 하나가 아니라 `_KRX_CALENDAR_SOURCES` 의 **합집합**이다. 지수 하나만
  보면 야후 피드에 구멍이 났을 때 그날이 그대로 휴장일이 된다 —
  2026-09-10(목)이 `^KS11`·`^KQ11` 양쪽에 없었지만 `005930.KS` 에는 있었다.
  이 집합은 "없으면 휴장"으로 읽히므로 그 구멍은 저장 가드가 그날 종가를
  **영구히** 거부하게 만들고, `last_completed_kr_session()` 은 그 전날을 가리켜
  수집기는 이미 최신이라고 믿는다. 소스끼리 어긋나면 경고를 남긴다.
- **오늘도 캘린더에 들어 있다.** 예전에는 아니었다 — 야후가 미확정 종가를
  `NaN` 으로 줬기 때문인데, 지금은 장중에도 실시간 Close 를 채워 준다.
  **외부 계약이 바뀌면 그걸 전제로 쓴 가드도 같이 바뀌어야 한다**
  (`dividendYield` 와 같은 유형). 종가 확정 여부는 이 집합이 아니라
  `last_completed_kr_session()` 이 15:30 KST 로 직접 판단한다.
- 그래도 **장중 게이트에는 `is_kr_extended_hours()` 를 쓴다.** 이 캘린더는
  관측 기반이라 첫 체결이 찍히기 전(08:30 시간외, 08:50)에는 오늘을 모른다.
- 캐시는 3단이다: 프로세스 메모 10분 → DB 2시간 → 소스. 메모가 없으면
  `save_prices_to_db` 가 **행마다** DB 를 왕복한다(2년 프레임에 992ms).
  DB 캐시 키에는 소스 구성이 들어간다 — 안 넣으면 소스를 고쳐도 옛 결과가
  2시간 동안 계속 나온다.
...
`ENABLE_SCHEDULER` 기본값은 `false` 다 (`main.py`). 켜는 것은 권장하지 않는다:
...
로컬도 마찬가지다. 병렬 창이 여럿이면 같은 수집이 창 수만큼 돌고 yfinance 는
IP 단위로 막는다 — 한 창이 걸리면 전부 죽는다. `dev.sh --scheduler` 로 켜되
한 번에 한 창만 (§7.7).
...
**두 구현이 같은지 볼 때는 진짜 런타임에 물어라.** 한쪽을 다른 언어로 옮겨 적고
그 전사본과 비교하면, **당신이 일관되게 읽었다는 것만 검증된다.** 잘못 읽은 동작은
양쪽에 똑같이 빠져 있어서 검사가 그걸 볼 수 없다.

> 사고: `formatCompact`(JS)와 `_fmt_amount`(파이썬)가 같은 값을 내는지 16,066건
> 대조해 불일치 0건이었는데, 대조 상대가 손으로 옮긴 파이썬 전사본이었다. 실제
> node 에 물으니 40건이 달랐다 — `toFixed` 는 자리구분을 안 넣고(구간마다 다르다),
> `.5` 를 올린다(파이썬 `format` 은 짝수로 내린다). 그리고 그 수정으로 넣은
> `floor(x*10**d + 0.5)` 가 **곱셈이 만드는 없던 동점** 때문에 246건을 새로 틀렸다.
> `Decimal(x).quantize(..., ROUND_HALF_UP)` 이 `toFixed` 를 재현한다.

프론트/백엔드가 같은 규칙을 구현하는 자리(통화 포맷, 시장 캘린더, 반올림)는 전부
이 검사가 필요하다. 대조 스크립트는 `scratchpad/cross_check_node.py` + `fc.js` 를
본으로 쓴다.
...
테스트: pytest <개수> passed / npm run build exit 0
요청: (남의 경로가 필요하면 여기에)
```
...
- **남의 테스트가 깨지면 "닿는 방법" 까지만 고친다.** 자기 소유 파일을 고쳐
  `backend/tests/` 가 **수집·setup 단계에서** 깨질 때 (monkeypatch 대상이 사라짐,
  언팩 arity 불일치, 대역의 반환 계약이 바뀜) 최소한의 기계적 수정을 **같은
  커밋에** 넣고 사후 통지한다. 승인을 기다리지 않는다.

  > **테스트가 코드에 닿는 방법은 고쳐도 되고, 테스트가 주장하는 내용은
  > 못 고친다.** assert 문구·기대값·케이스 추가/삭제는 소유자 몫이다.

  판정은 기계적이다 — diff 에 assert 줄이 있는가. 예외가 하나 있다:
  **원장·xfail 마커처럼 스스로 제거를 요구하는 장치**는 그 요구를 따르는 것이
  곧 규약이라, 버그가 고쳐져 XPASS 가 뜨면 마커를 지우는 것이 맞다. 그 장치가
  요구할 때만 해당한다.

  (이 규칙이 생긴 이유: 하루에 네 번 났고 매번 통합 승인을 기다리는 동안
  `main` 이 빨간 채로 남았다. 그게 기계적 한 줄 수정보다 나쁘다.)

- **게이트가 두 브랜치에 걸쳐 있으면** 빨간 채로 커밋하고 사유를 명시한다.
  조건 셋이 **전부** 맞을 때만이다: ① 빨간 원인이 자기 코드가 아니고 자기가
  고칠 수도 없다 ② 자기 커밋이 먼저 있어야 소유 창이 고칠 수 있다 ③ 통합이
  둘을 같이 받아 초록을 확인한다. 하나라도 빠지면 성립하지 않으므로 다시 묻는다.
  커밋 메시지 첫 줄에 표시한다 — 나중에 이분법으로 원인을 찾는 사람이 그
  커밋에서 멈춘다.

- `CLAUDE.md` 는 통합만 **편집**한다. 다섯 창이 전부 규칙을 추가하고 싶어하고 매
  병합마다 충돌하기 때문이다. 이건 편집권 이야기지 **규칙이 옳다는 뜻이 아니다** —
  §7.9 를 보라.
...
`main.py` 의 `ENABLE_SCHEDULER` 기본값은 **`false`** 다. 원래 `true` 였는데, 리포
전체에서 `true` 로 세팅하는 실행 경로가 하나도 없었다 — `Dockerfile` 은 `false`,
`dev.sh` 는 `--scheduler` 를 줬을 때만 `true`. 아무도 원하지 않는 기본값이
환경변수를 주지 않은 사람만 함정에 빠뜨리고 있었다: 그대로 두면 기동할 때마다
...
**이 문서의 규칙을 무조건 따르지 마라.** 여기 적힌 것은 그때까지 알던 최선이지,
검증된 진리가 아니다. 더 나은 방법이 보이면 바꾼다.

다만 바꾸는 근거는 **증거**여야 한다. 취향이나 일반론이 아니다.

- **바꿔도 되는 것**: 코드를 읽어보니 규칙이 서술하는 상황이 더는 존재하지 않는다.
  규칙대로 했더니 실제로 문제가 생겼다. 같은 목적을 더 잘 달성하는 방법을 찾았고
  왜 더 나은지 말할 수 있다.
- **바꾸면 안 되는 것**: "보통은 이렇게 한다", "이게 더 깔끔하다", 규칙의 이유를
  모르겠어서.

§1 의 규칙 상당수는 실제 사고의 흉터다 — 인용된 사고를 먼저 읽어라. 이유를 알고도
틀렸다고 판단되면 그때 바꾼다. 이유를 모른 채 걷어내면 그 사고가 돌아온다.

절차: 역할 창은 편집하지 않고 통합에 **근거와 함께** 제안한다. 통합이 확인하고
반영한다. 반영할 때 **무엇을 보고 바꿨는지**를 남긴다 — 다음 사람이 같은 판단을
다시 하지 않도록.

#### 숫자를 문서에 박지 마라

`163 passed` 같은 기준값을 문서·프롬프트·스크립트에 적으면 조용히 낡는다. 테스트를
하나 추가한 순간 다섯 창의 문서가 전부 틀린 값을 들고 있게 되고, 아무도 그걸
알려주지 않는다. §1.3 과 같은 결함이다 — 틀렸다는 신호가 없다.

기준값이 필요하면 **재는 방법**을 적어라. 비교가 필요하면 그때 양쪽을 직접 재서
비교한다.
...
- ~~`SCHEDULER_AUDIENCE` 미확인~~ — 2026-09-11 확인했다. Cloud Run 에
  `SCHEDULER_AUDIENCE`(서비스 URL)와 `SCHEDULER_SERVICE_ACCOUNT` 가 둘 다
  설정돼 있어 내부 수집 엔드포인트의 토큰 audience 검증이 동작한다.
  `FIREBASE_AUTH_EMULATOR_HOST` 가 운영 환경변수에 **없는 것**도 함께 확인했다.
- 프로세스 내 스케줄러의 tier1 게이트가 미국 전용 — 켤 계획이면 시장별로 분리 필요.
  (`seed_closing_prices` 의 미국 전용 필터는 고쳤다. 나머지 게이트는 그대로다.)
